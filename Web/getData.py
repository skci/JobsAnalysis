...
def Spider(key):
    ...
    for response in responses:
        ...
        for li in lis:
            ...
            dit = pd.DataFrame(dit, index=[0])
            df = pd.concat([df, dit], ignore_index=True)
...
def Clean(df):
    ...
    # 读取数据库信息
    sql_df = pd.read_sql('SELECT * FROM liepin', con=con)

    # 合并数据集>>>新增数据和数据库原数据
    concat_df = pd.concat([sql_df, clean_df], axis=0, ignore_index=True)

    duplicate_bool = concat_df.duplicated(keep='first',
                                          subset=['job', 'tag', 'company', 'company_tag', 'address', 'city', 'work',
                                                  'edu', 'salary_low', 'salary_high'])

    # 新增数据集对比数据库的重复部分
    repeat = concat_df.loc[duplicate_bool == True]

    # 合并数据集>>>新增数据和重复数据>>>目的：所有重复标记为True
    clean_df = pd.concat([clean_df, repeat], axis=0, ignore_index=True)
    ...
    # 存入数据库
    clean_df.to_sql(name='liepin', con=con, chunksize=100, method='multi', index=False,
                    if_exists='append')  # 'fail'不存储，'replace'替换，'append'追加
    con.close()
